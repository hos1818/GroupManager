...
def extract_user_and_text(message: Message, args: List[str]) -> (Optional[int], Optional[str]):
    ...
    if entities and ent and ent.offset == len(message.text) - len(text_to_parse):
        ...
        if not user_id:
            message.reply_text("ليس لدي هذا المستخدم في قاعدة البيانات الخاصة بي. ستكون قادرًا على التفاعل معهم إذا "
    ...
    try:
        ...
        if excp.message in ("User_id_invalid", "Chat not found"):
            message.reply_text("لا يبدو أنني قد تفاعلت مع هذا المستخدم من قبل - يرجى إعادة توجيه رسالة منة "
                               "لمنحي السيطرة! (أحتاج قطعة منهم لأتمكن من "
                               "لتنفيذ الأوامر ...)")
        else:
