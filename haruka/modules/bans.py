...
def ban(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if user_id == bot.id:
        message.reply_text(tld(chat.id, "لن احظر نفسي ، هل أنت مجنون؟"))
        return ""

    if is_user_ban_protected(chat, user_id, member):
        message.reply_text(tld(chat.id, "لماذا سأحظر مشرف؟ هذه تبدو فكرة غبية جدا."))
        return ""
    ...
    try:
        ...
        if excp.message == "Reply message not found":
            ...
            LOGGER.exception("ERROR banning user %s in chat %s (%s) due to %s", user_id, chat.title, chat.id,
                             excp.message)
            message.reply_text(tld(chat.id, "اللعنة ، لا يمكنني حظر هذا المستخدم."))
...
def temp_ban(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member):
        message.reply_text(tld(chat.id, "هذا المستخدم محمي ، مما يعني أنه لا يمكنك حظر هذا المستخدم!"))
        return ""
    ...
    if not reason:
        message.reply_text(tld(chat.id, "لم تحدد وقتًا لحظر هذا المستخدم!"))
        return ""
    ...
    try:
        ...
        #bot.send_sticker(chat.id, BAN_STICKER)  # banhammer marie sticker
        message.reply_text("تم الحظر! سيتم حظر المستخدم ل {}.".format(time_val))
        return log
        ...
        if excp.message == "Reply message not found":
            # Do not reply
            #bot.send_sticker(chat.id, BAN_STICKER)  # banhammer marie sticker
            message.reply_text(tld(chat.id, "تم الحظر! سيتم حظر المستخدم ل {}.").format(time_val), quote=False)
            return log
        else:
            LOGGER.warning(update)
            LOGGER.exception("ERROR banning user %s in chat %s (%s) due to %s", user_id, chat.title, chat.id,
...
def kick(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if user_id == bot.id:
        message.reply_text("أنا لا أطرد نفسي!")
        return ""

    if is_user_ban_protected(chat, user_id):
        message.reply_text("لماذا أقوم بطرد مشرف؟ تبدو فكرة غبية جدا.")
        return ""
    ...
    if res:
        ...
        message.reply_text("اللعنة ، لا يمكنني طرد هذا المستخدم.")
...
def kickme(bot: Bot, update: Update):
    ...
    if is_user_admin(update.effective_chat, user_id):
        update.effective_message.reply_text("لماذا أقوم بطرد مشرف؟ تبدو فكرة غبية جدا.")
        return

    res = update.effective_chat.unban_member(user_id)  # unban on current user = kick
    if res:
        update.effective_message.reply_text("ليس هناك أى مشكلة.")
    else:
        update.effective_message.reply_text("هاه؟ لا استطيع :/")
...
def banme(bot: Bot, update: Update):
    ...
    if is_user_admin(update.effective_chat, user_id):
        update.effective_message.reply_text("لماذا سأحظر مشرف؟ تبدو فكرة غبية جدا.")
        return

    res = update.effective_chat.kick_member(user_id)  
    if res:
        update.effective_message.reply_text("لا مشكلة ، تم الحظر.")
        log = "<b>{}:</b>" \
              "\n#BANME" \
              "\n<b>User:</b> {}" \
...
def unban(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if is_user_in_chat(chat, user_id):
        message.reply_text("لماذا تحاول إلغاء حظر شخص موجود بالفعل في المجموعة؟")
        return ""
...
def sban(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    try:
        ...
        if excp.message == "المستخدم ليس موجود":
            return ""
        else:
            raise
